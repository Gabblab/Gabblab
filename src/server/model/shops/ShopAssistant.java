	...
	public void handleOtherShop(int itemID) {
		if (c.myShopId == 20) {
			if (c.pcPoints >= getSpecialItemValue(itemID)) {
				...
				c.sendMessage("You do not have enough CXP points to buy this item.");
			}
			} else if (c.myShopId == 25) {
				if (c.dungpoint >= getSpecialItemValue(itemID)) {
					if (c.getItems().freeSlots() > 0){
						c.dungpoint -= getSpecialItemValue(itemID);
						c.getItems().addItem(itemID,1);
						c.getItems().resetItems(3823);
					}
					} else {
						c.sendMessage("You need more Dungeoneering points to purchase this item.");	
					}
